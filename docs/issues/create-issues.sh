#!/usr/bin/env bash
# Creates all build improvement issues in GitHub.
# Prerequisites: gh CLI authenticated (gh auth login)
# Usage: bash docs/issues/create-issues.sh

set -euo pipefail

REPO="fmueller/fabulae"

# Ensure labels exist (creates if missing, no-op if they exist)
...
gh label create "build"              --repo "$REPO" --color "0e8a16" --description "Build command and prose generation"        2>/dev/null || true
gh label create "prompt-engineering"  --repo "$REPO" --color "d4c5f9" --description "LLM prompt quality and instruction tuning" 2>/dev/null || true
...
gh issue create --repo "$REPO" \
  --title "Bug: Pipeline selection always defaults to sequential" \
  --label "bug,build" \
  --body "$(cat <<'EOF'
## Description
...
echo "Created issue 1: Pipeline selection bug"

# --- Issue 2 ---
gh issue create --repo "$REPO" \
  --title "Scene summaries used as titles — produces bloated headers" \
  --label "enhancement,build,prompt-engineering" \
  --body "$(cat <<'EOF'
## Description
...
echo "Created issue 2: Scene summaries as titles"

# --- Issue 3 ---
gh issue create --repo "$REPO" \
  --title "Build prompts produce insufficient dialogue" \
  --label "enhancement,build,prompt-engineering" \
  --body "$(cat <<'EOF'
## Description
...
- **Standard scene prompt** (`prompts.py:117`): Only one line mentions dialogue: `"Show character emotions and reactions through action and dialogue"` — frames dialogue as optional.
...
echo "Created issue 3: Insufficient dialogue"
...
1. **Add scene-level word count guidance** to the user prompt. Calculate a target from:
   ...
   - A format-based default (e.g., novel scenes ~1500-2500 words, short-story scenes ~800-1500 words, novella ~1000-2000 words)
...
echo "Created issue 4: No word count targets"
...
echo "Created issue 5: Continuity summaries"

# --- Issue 6 ---
gh issue create --repo "$REPO" \
  --title "Enhanced mode hooks generated but never rendered in output" \
  --label "enhancement,build" \
  --body "$(cat <<'EOF'
## Description
...
echo "Created issue 6: Hooks not rendered"

# --- Issue 7 ---
gh issue create --repo "$REPO" \
  --title "Add 'show don't tell' and prose craft instructions to build prompts" \
  --label "enhancement,build,prompt-engineering" \
  --body "$(cat <<'EOF'
## Description
...
echo "Created issue 7: Prose craft instructions"

# --- Issue 8 ---
gh issue create --repo "$REPO" \
  --title "Short-story scene headers should use scene breaks, not titled sections" \
  --label "enhancement,build" \
  --body "$(cat <<'EOF'
## Description
...
1. **Format-aware scene separation**: In `_combine_scenes()` and the writer, detect the format:
   - `novel` / `novella`: Keep `## Title` headers (but fix the title — see related issue for scene summaries as titles)
...
2. **Format-specific word count defaults**: The word count targets should also vary by format.
...
echo "Created issue 9: Per-format prose differentiation"

# --- Issue 10 ---
gh issue create --repo "$REPO" \
  --title "Standard scene prompts significantly weaker than enhanced" \
  --label "enhancement,build,prompt-engineering" \
  --body "$(cat <<'EOF'
## Description

The standard (non-enhanced) scene prompt is significantly weaker than the enhanced version:
...
echo "Created issue 10: Standard prompts weaker than enhanced"
