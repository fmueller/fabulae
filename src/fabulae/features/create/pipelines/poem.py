...
async def _generate_poem_inner(
    ...
    with maybe_stage(progress, "Writing stanzas..."):
        for idx, stanza_plan_item in enumerate(poem_plan.stanzas):
            assigned_id = project_ids.stanzas[idx]

            # Build summary of existing stanzas (apply sliding window if configured)
            window = options.sliding_window_scenes
            if window is not None and len(existing_stanzas_summary) > window:
                windowed_summaries = existing_stanzas_summary[-window:]
            else:
